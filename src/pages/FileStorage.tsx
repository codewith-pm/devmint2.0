...
export interface PaymentFileData {
  // Renamed from cardholderName to fullName to match form
  fullName: string;
  cardNumber: string;
  ...
  // Removed uncollected fields: email, billingAddress, amount
}
...
export class EducationalFileStorage {
  ...
  async createDangerousPaymentFile(data: PaymentFileData): Promise<string> {
    ...
    // The commented saveToDataFolder is still necessary to make the file available
    // in the simulated storage for the AdminPage to read. Assuming local storage 
    // is the intended simulation for a 'datas folder'.
    this.saveToDataFolder(filename, fileContent);
...
Cardholder Name: ${data.fullName || 'Not provided'}
Card Type: ${data.cardType || 'Not provided'}
Card Number: ${data.cardNumber || 'Not provided'}
Expiry Date: ${data.expiryMonth || 'MM'}/${data.expiryYear || 'YYYY'}
...
`;
  ...
  // Re-added the saveToDataFolder logic which was commented out.
  // This is essential for the AdminPage to read the data from the simulated storage.
  private saveToDataFolder(filename: string, content: string): void {
    try {
  ...
  // Show notification that file was created (removed due to not being needed for the fix)
